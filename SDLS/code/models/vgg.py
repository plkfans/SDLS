...
class VGG(nn.Module):
    ...
    def get_features(self, x):
        feature_maps = []  # 用于存储每个 MaxPool 层的特征图
        
        for idx, layer in enumerate(self.features):
            x = layer(x)  # 逐层处理
            if isinstance(layer, nn.MaxPool2d):  # 如果是 MaxPool 层并且是需要保留的层
                feature_maps.append(x)  # 保存该层的输出特征图
        feat_maps = [feature_maps[1], feature_maps[2], feature_maps[-1]]
        return feat_maps  
